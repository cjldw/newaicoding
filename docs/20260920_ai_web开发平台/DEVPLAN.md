...
> ARCH:./ARCH.md
> 创建日期:2026-09-21
> 状态:**已确认**(增量3/4/5 收口见各行;**增量6(R32)计划已确认**,2026-09-24,A1–A7 用户按推荐锁定;待确认清单空)

## 需求概述

从零搭建一个**AI 驱动的研发流程协作平台**,覆盖**需求打磨 → 开发 → 测试 → 发布 → 归档**全流程;32 个需求点(R1–R32),10 个模块(M1–M10),全部新建,无历史代码包袱。
...
**当前进度: 全部 32/32 (100%) - 增量6 R32 完成待提交确认;R27 判据 5 仍待用户重录 GitLab bot token 复验 | rd-fix 第 26 轮收口 | 引导进入 /rd-check**
...
| R32 | Runner 标签管理与全字段编辑(增量6) | M5 | ✅(fixed;pytest 21/21 新增+波及面 102 零回退,tsc/build 零错,code-review 双轴 4 修 3 拒;真机 E2E 归 rd-test) | ./DEVPLAN/R32.md |
...
| **增量6(2026-09-24)** | R32(Runner 标签管理与全字段编辑,M5) | 依赖 R16(调度)/R31(快建弹窗)/R19/R25;runners 表加 tags JSON 列 | 1.5d | 14(调度过滤与编辑端点可并行) |
...
| 2026-09-24 | **增量6 R32 计划落盘(rd-plan)**:R32 Runner 标签管理与全字段编辑——tag 固定枚举 requirement/dev/test(Q58),调度候选过滤「tags 含任务类型 或 空=兜底」(pick_runner_db 应用层过滤,least-load 不变;无匹配 8003 文案不变+日志细分 Q61);release 仍 role=deploy 不看 tag;编辑除 role/token 外全字段(Q59:名称唯一 16005/最大容器数调小允许存量不动/tags;deploy 行公网 IP);R31 快建弹窗同步 tag 多选(Q60);新增 runner.update 审计(Q62);新增 16008(标签非法/deploy 带标签)+PATCH 端点+runners.tags JSON 列(存量 NULL 兼容读)。分片 DEVPLAN/R32.md(格式自检 6/6 过)。**自动确认**:调度算法应用层过滤(量小无需 SQL JSON)、前端 chips/多选(既有 chip/checkbox 形态)、deploy 调度零变化。**待人工确认 5 项已全部确认(A1–A7 按推荐锁定)**:A1 deploy 带标签 16008 显式拒绝 / A2 非法标签 16008(HTTP 200 统一响应;PRD"400"系口径笔误已留痕)/ A3 disabled 允许编辑 / A4 name 未变跳过查重 / A7 tags JSON 可空列+NULL 兼容读(A5 并发快照/A6 调小允许 PRD 已拍板)。**DEVPLAN 置已确认,待 /rd-dev** | rd-plan 增量6 |
...
| 2026-09-24 | **R32 执行完成(增量6 收官,全计划 32/32 需求点代码侧完成)**:① 后端——runners.tags JSON 可空列(alembic d8e4f2a6b9c3,dev 库已 upgrade/测试库手工补列)+ 16008 + validate_tags(单遍校验去重/deploy 非空拒)+ pick_runner_db 应用层过滤(task_tag;无匹配细分日志"等待 {tag} 专属 Runner")+ schedule_and_start/create_polish_task("requirement")/start_task(按 type 透传)三级透传 + PATCH /api/admin/runners/{id}(name 未变跳查重/disabled 可编辑/role·token·is_local 不可改/deploy 公网 IP 置空 2007 拒)+ 两个 create 加 tags + runner.update 审计(changed/before_tags/after_tags,no-op 不落);② 前端——TAG_OPTIONS/TagPicker 三弹窗复用/编辑弹窗(deploy 行隐藏 tags 显提示)/「任务类型」列 chips(空=通用)/useUpdateRunner;③ tdd——QA 红测 21 用例(Red 实证)→ Green 全绿;波及面 10 文件 102 passed 零回退;runner 侧零改动;④ 收口 code-review 双轴(medium):7 CONFIRMED → 4 修(枚举单源化 ALLOWED_TASK_TAGS/热路径 set 分配根除/no-op PATCH 不落空审计/validate_tags 合单遍)3 拒(markdown.ts 越界登记/惯用法提取 churn>价值/微优化反增分支),修复后 21/21 复绿。**编排留痕**:QA/后端 subagent autocompact 死亡(本会话 3 派 3 死),按先例主 agent 接管(QA Red 代跑补档/后端修正收尾);前端 subagent 存活(tsc 零错+渲染核对 7/7)。**决策留痕**:① 死掉的 backend subagent 版 update_runner 允许改 role(越界)+before 快照丢失+16005 文案不符,接管时全部纠正;② 远程 create_runner 名称冲突用 13002 与 16005 口径分裂系存量不一致(R32 无测试覆盖,不混入本轮,登记 ISSUES 口径);③ QA 红测机械缺陷 4 处修正(_make_runner 缺 created_by/Requirement 缺 req_branch/LLM 解析未打桩/start_task 第 4 参误传 None——照 test_r8f4 先例);④ PATCH tags 显式 null=清空(通用兜底),未传=不改动(exclude_unset);⑤ **越界发现登记**:frontend/src/utils/markdown.ts 共享渲染器丢围栏代码块/斜体/有序列表支持(code-review #2,非 R32 触碰面,转 ISSUES 口径)。**未提交待用户确认**;真机 E2E(A[dev]+B[] 专属分配落位)归 rd-test | rd-dev 增量6 R32 |
