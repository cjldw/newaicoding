...
### R31:Runner 本地快速创建与本机生命周期管理

- **描述**:Runner 管理页(超管)新增「快速创建(本机)」:平台在**本机直接拉起 runner 进程**并自动注册上线,一键完成"创建 token → 启动 → 上线"全流程;本地 runner 获得**停止 / 重启 / 删除(代停容器)**三个生命周期能力,解决开发机与单机私有化部署手动起 runner 繁琐、删除被运行中容器 16001 卡死的问题(2026-09-24 增量5,Q51–Q57)
- **触发场景**:超管在「平台管理 → Runner 管理」点击「快速创建(本机)」;适用于开发调试与平台/Runner 同机的单机私有化部署
- **前置条件**:R16 Runner 管理机制已存在;平台以源码方式运行且能定位到仓库 `runner/` 目录;本机 Python ≥3.10 且已安装 `docker`、`websockets` 依赖;本机 Docker daemon 可用(否则 runner 无法管理容器)
- **边界定义**:
  - 做什么:
    - **快速创建**:最小弹窗(名称默认 `local-<runner_id前8位>` 可改 + 最大容器数默认 10)→ 后端自动完成:生成 runner 记录(worker)+ token  bcrypt hash → 以子进程方式启动 `python runner/main.py`(注入 `PLATFORM_URL`/`RUNNER_TOKEN`/`RUNNER_ROLE`/`RUNNER_ID`)→ runner 主动 WS 注册 → 列表 online。**token 对用户不可见**(平台内部注入;手动排障场景走「重置 token」)
    - **本机 runner 标识**:`is_local` 标记;列表按 `启动方式` 区分(token 型 / 本机型),本机型才有 停止/重启/删除代停 能力
    - **停止(指令式)**:平台经既有 WS 通道下发 `runner_shutdown`,runner 优雅自退;进程退出后记录标 offline。平台重启后仍可用(不依赖 PID 隶属关系)
    - **重启**:停止 + 重新 spawn
    - **删除代停(仅本机 runner)**:删除时平台先对该 runner 全部 creating/running 容器下发停止(复用 R4/R16 既有 stop 流程 = **销毁前强制 push 未推提交**),全部停止后停进程并删记录;任一容器 push 失败 → **删除中止报错**(不静默丢代码、不半删)。远程 token 型 runner 维持原语义(有容器 → 16001 拒绝)
    - **环境校验**:点击「快速创建」时后端逐项校验(runner 代码存在 / Python 可执行 / 依赖可 import / Docker 可用),失败返回**细分错误文案**(如"未检测到 runner 代码目录,仅支持平台源码方式部署")
    - **数量上限**:本机快速创建的 runner ≤ **3 个**,超限 16004 拒绝
  - 不做什么:
    - **不做**远程 runner 的进程管理(平台不跨机 SSH/agent 拉停;远程 runner 维持 token 手动运维)
    - **不做**平台重启后自动重拉本地 runner(runner 自身 WS 无限重连已覆盖"平台重启后自动回来";**平台机器重启后的自启动** V1 不做,需手动点一次启动/V2 再议)
    - **不做** Docker 容器形态的本地快速启动(不依赖 `platform/runner:v1` 镜像构建;V1 仅 spawn 宿主进程)
    - **不做** deploy 角色快速创建(本机 runner 恒 worker;deploy 需公网 IP 语义,走原 token 流程)
    - **不做**并发任务配额联动(3 个上限为固定值,不入平台设置)
- **字段定义**:
  | 字段 | 类型 | 必填 | 默认值 | 校验规则 | 说明 |
  |---|---|---|---|---|---|
  ...
  | launch_command(响应) | json | — | — | — | 创建响应返回实际命令与环境变量名(**不含 token 值**),供审计/排障 |
- **交互规则**:
  | 场景/条件 | 行为/规则 | 结果/去向 |
  |---|---|---|
  | 超管点「快速创建(本机)」 | 弹窗(名称+最大容器数)→ 确认 | 后端校验 → 通过则 spawn |
  | spawn 成功 | 记录 online(等 WS 注册)→ 列表轮询/即时刷新显示 online 新行 | 新行高亮 |
  | 环境校验失败 | 返回细分错误码+文案 | 停留页面,错误区展示 message |
  | 本机 runner 已有 3 个 | 16004 拒绝"本机快速创建的 Runner 已达上限(3)" | 不 spawn |
  | 点「停止」 | WS 下发 runner_shutdown;runner 自退;记录 offline | 按钮态:online→offline 变「启动」 |
  | 点「重启」 | 停止 + 重新 spawn(复用原 token) | 重新上线 |
  | 点「删除」(本机 runner,有容器) | 代停全部容器(push 后销毁)→ 停进程 → 删记录 | 全链路成功才删;任一失败中止 |
  | 点「删除」(本机 runner,无容器) | 停进程 → 删记录 | 删除成功 |
  | 点「删除」(远程 runner,有容器) | 16001 拒绝(原语义不变) | 提示先处理容器 |
  | 平台重启 | 本地 runner 进程独立存活,WS 自动重连(既有指数退避) | 列表回 online |
- **依赖**:R16(Runner 管理/WS 协议/调度)、R4/R16(容器 stop+强制 push)、R19(超管守卫/审计)
- **异常与边界场景**:
  - **spawn 成功但注册超时**(10s 内未 online,如 Docker daemon 中途挂):记录保留(offline/failed),错误提示"Runner 已启动但未完成注册,可查看启动日志或重置 token 后手动启动";**不**自动回滚删除(给用户排障留入口,可停止/重启/删除)
  - **runner 进程崩溃**:平台不自动重拉(V1);下次心跳超时标 offline;管理页显示 offline,用户可点「启动」重新拉起
  - **重复 spawn 防护**:同一 runner online 时再点「启动」→ 幂等返回"已在运行";快速连点加锁
  - **停止指令无回执**(runner 卡死/WS 半死):超时 5s 后平台强制终止子进程(本机 runner 有进程句柄,可安全 kill——与"平台重启后杀孤儿 PID"不同,句柄在手才允许强杀)
  - **代停中某容器 push 失败**:整删中止,错误文案指明哪个容器 push 失败;用户先处理该容器(重试/强制停止入口)再删
  - **name 冲突**:沿用 R16 唯一校验,16003
  - **安全**:runner 进程继承平台进程环境(读 backend/.env 权限)——V1 假设单管理员信任模型(平台=超管本机);多租户场景不适用本功能,文档标注
  - **审计**:`runner.create_local`(记录 name/role,不记 token/命令值)、`runner.stop`、`runner.start`、`runner.delete`(R25 模式接入)
- **验收标准**:
  1. 超管在本机(平台源码运行)点「快速创建」→ 弹窗填名称 → 确认 → 10s 内列表出现 online 的 worker runner,全程无需手动复制 token/开终端
  2. 环境缺项(runner 代码不存在 / Python 依赖缺 / Docker 不可用 / 非源码部署)→ 点击即返回对应细分错误文案,不落库不 spawn
  3. 已 3 个本机 runner 时第 4 次创建 → 16004 拒绝
  4. 「停止」→ 进程退出、记录 offline;「重启」→ 重新上线;online 中点启动幂等
  5. 删除带 2 个 running 容器的本机 runner → 容器被停(工作区有未推提交的先推后销)、进程停、记录删;某容器 push 失败 → 中止且 runner 保留,错误指明容器
  6. 平台重启后:本地 runner 自动重连回 online;「停止」按钮(指令式)仍可用
  7. 远程 token 型 runner 行为零回归(无停止/重启按钮;删除有容器仍 16001)
  8. 创建/停止/启动/删除事件入审计日志;**任何响应与审计 detail 不含 token 值**

### R32:Runner 标签管理与 Runner 全字段编辑

- **描述**:Runner 管理新增**任务类型标签(tag)**——超管给 runner 打 requirement(需求)/dev(开发)/test(测试)标签,调度器按"任务类型↔同名 tag"匹配分配(需求任务用打了 requirement 的 runner,开发任务用打了 dev 的 runner);未打任何 tag 的 runner **兜底接所有 worker 任务**(存量零影响)。同时新增 **Runner 编辑**能力:除 role/token 外全字段可改(名称/最大容器数/tags;deploy 行加公网 IP)。R16 曾把"标签匹配"列为 V2 范围外,本需求提前落地其任务类型部分(2026-09-24 增量6,Q58–Q62)
- **触发场景**:超管在 Runner 管理页新建/编辑 runner 时勾选任务类型标签;调度器在任务拉起容器时按 tag 匹配
- **前置条件**:R16 Runner 管理/调度;R19 超管;R31(快速创建弹窗同加 tag 多选)
- **边界定义**:
  - 做什么:
    - **tag 模型**:固定枚举 `requirement / dev / test` 三值;一个 runner 可打 0~3 个;`tags=[]`(空)= 兜底接**所有** worker 任务(存量行默认空=行为零变化)
    - **调度匹配**(worker 域):候选 = `status=online AND role=worker AND 容量未满 AND (tags 含任务类型 OR tags 为空)` → 最少负载(既有算法不变,仅候选过滤加一层);无匹配 → None → 既有 8003 排队语义不变
    - **release 不参与 tag**:仍按 `role=deploy` 选(公网 IP 语义),tag 对 deploy 无意义
    - **tag 编辑**:新建对话框(远程+R31 快速创建)加 tag 多选(默认全不勾);操作列新增「编辑」弹窗——除 role 外全字段可改:名称(全平台唯一校验)/最大容器数/tags;deploy 行加公网 IP;**token/role/is_local 不可改**
    - **列表展示**:新增「任务类型」列,tag chips 渲染;空 tags 显示「通用」(兜底语义可视化)
    - **审计**:新增 `runner.update` 事件,detail 记 `{before_tags, after_tags, 改动字段列表}`(无敏感值)
    - **R31 快速创建弹窗**同步加 tag 多选(默认全不勾)
  - 不做什么:
    - 不做自由标签/GPU 等资源标签(R16 V2 项余下部分仍范围外)
    - 不做"任务类型→tag"映射配置(同名同名,无映射层)
    - **role 不可编辑**(worker↔deploy 语义不同,换角色=删了重建);token/is_local 不可编辑
    - 无匹配 runner 时排队文案不变(调度日志细分"等待 {类型} Runner",前端不感知)
- **字段定义**:
  | 字段 | 类型 | 必填 | 默认值 | 校验规则 | 说明 |
  |---|---|---|---|---|---|
  | tags | JSON(list[str]) | 是 | `[]` | 值 ⊆ {requirement,dev,test};去重;空数组=兜底 | runner 可接的任务类型 |
  | 编辑-名称 | string(64) | 是 | - | 全平台唯一(16005 既有) | 与创建同校验 |
  | 编辑-最大容器数 | int | 是 | - | 1-100 | 调小允许,存量容器不动(新调度按新值卡容量) |
...
- **交互规则**:
  ...
  | 编辑保存 | 名称唯一校验(16005)+ tags 合法性;`runner.update` 审计 | 列表刷新,新 tag 下次调度即生效(调度实时查库,无需重启 runner) |
  | max_containers 调小 | 允许;存量容器不动 | 新调度按新值卡容量 |
  | 存量 runner(未打 tag) | tags=[] 兜底接所有 worker 任务 | 行为零变化 |
- **依赖**:R16(调度/管理页)、R31(快速创建弹窗)、R19(超管)、R25(审计)
- **异常与边界场景**:
  - 某类型专属 runner 全部离线:任务 8003 排队(既有语义);调度日志细分"等待 {类型} Runner"
  - 编辑时名称与他 runner 冲突 → 16005;tags 含非法值 → 2007 口径校验拒绝
  - runner 被编辑 tag 的同时有任务在调度:调度实时查库,以查询瞬间快照为准(无需加锁,单行 UPDATE 原子)
  - 本机 runner(is_local)同样可编辑 tags/名称/最大容器数;token/启停不受编辑影响
  - deploy 行编辑:public_ip 必填校验与创建一致
  - 打磨任务(type=requirement)与 dev/test 同受 tag 匹配约束
- **验收标准**:
  1. runner A tags=[dev]、B tags=[]:dev 任务落 A;requirement 任务落 B(兜底);release 任务落 deploy runner(A/B 均不参与)
  2. A/B/C(tags=[test]) 三机:dev 任务永不落 C;A 离线时 dev 任务排队 8003,C 不接(无兜底——C 有 tag 即专属)
  3. 编辑弹窗改 tags/名称/最大容器数保存 → 列表即显;名称冲突 16005;`runner.update` 审计含前后 tags
  4. 快速创建勾选 tags → 新 runner 带标签参与匹配
...
- **描述**:AI 对话输入框支持 `/` 唤起**能力菜单**——分组列出项目 Skills、平台公共 Skills(R17)与已启用的 MCP server(R17);选中后以斜杠原文(`skill-name`/`mcp-name`)随消息**透传**给容器内 claude CLI **原生执行**,平台不做任何技能解释/编排(Claude Code 的 skills/commands 本就是 CLI 原生机制,容器内预装 CLI)。交互与既有 `@` 文件补全同款(2026-09-24 增量7,Q63–Q67)
- **触发场景**:开发者在任务工作台执行任务时,想以 Claude Code 同款方式调用某个技能或 MCP 能力
- **前置条件**:R17(Skills 与 MCP 配置存储);R5(对话链路,任务容器运行中)
- **边界定义**:
  - **做什么**:`/` 触发下拉菜单(分组:项目 Skills / 平台 Skills / MCP);继续输入按名称过滤;↑↓ 导航、Enter 选中、Esc 关闭;与 `@` 文件补全互斥切换(按首字符 `@`/`/` 分流);选中插入斜杠原文,随消息原样发送;无 Skills 且无 MCP 时不弹菜单(原文发送)
  - **不做什么**:平台不解析/编排/执行技能与 MCP(执行全归容器内 CLI);不做 MCP **工具级**枚举(V2);终端不做斜杠补全(终端里手敲 CLI 原生 `/`);不做自定义斜杠命令的管理界面
- **字段定义**(菜单项;数据全部来自 R17 既有存储/接口,零新表):
  | 字段 | 类型 | 必填 | 默认值 | 校验规则 | 说明 |
  |---|---|---|---|---|---|
  | type | enum | 是 | - | skill / mcp | 分组依据 |
  | name | string | 是 | - | 非空 | 斜杠调用的名称原文 |
  | source | enum | 是 | - | project / platform | Skills 来源(MCP 恒 project 配置) |
  | description | string | 否 | - | - | 菜单副标题(既有字段,缺失则不显示) |
- **交互规则**:
  | 场景/条件 | 行为/规则 | 结果/去向 |
  |---|---|---|
  | 输入框首字符 `/` | 弹出能力菜单(按类型分组) | 下拉展示 |
  | `/` 后继续输入 | 按名称前缀/包含过滤 | 列表实时过滤 |
  | Enter/点击菜单项 | 插入 `name` 到输入框(替换 `/` 及已输过滤串) | 可继续追加消息文字 |
  | Esc / 点击外部 | 关闭菜单 | 输入保留 |
  | 无任何 Skills 且无启用 MCP | `/` 不弹菜单 | 原文发送 |
- **依赖**:R17(Skills/MCP 数据)、R5(对话);波及 R4(工作台)
- **异常与边界场景**:
  - 容器内 claude CLI 对 headless(`-p`)模式下 slash 语法的支持度:**rd-plan 期做兼容性验证**(事实核查),不支持则降级方案(菜单选中转为引导性提示文案)在 plan 期定
  - R17 既有接口字段若不足以支撑菜单(如缺启用状态/描述),plan 期补只读接口(见待确认清单)
  - 消息以斜杠原文透传,平台不校验名称存在性(CLI 侧对未知命令自有提示)
- **验收标准**:
  1. 任务工作台 AI 对话输入 `/` 弹出分组菜单(项目 Skills/平台 Skills/MCP),过滤与键盘导航可用
  2. 选中 skill 发送后,容器内 claude 按该 skill 执行(以回复内容可辨认为准)
  3. 选中 MCP server 发送后,回复体现该 server 参与执行
  4. 无 Skills/MCP 时 `/` 不弹菜单,消息正常发送

### R34:终端直接进入当前 AI 对话会话(验收确认型)

- **描述**:任务工作台打开终端 → 直接进入与 AI 对话面板**同一 claude 会话**(带完整历史上下文)。BUG-037/R9.F1 已实现该行为(`tasks.claude_session_id` + `--session-id/--resume` + 终端 exec 自动进 claude 包装),当时标记"verified 待真实容器浏览器实测"——**本需求点为验收确认型**:在真实容器环境实测其生效;实测若暴露缺陷,转 rd-fix 缺陷单修复,不在本需求点内扩代码范围(2026-09-24 增量7,Q68–Q70)
- **触发场景**:开发者想在终端里以 CLI TUI 方式继续与 AI 的当前对话
- **前置条件**:R9.F1(既有实现);R8(任务容器运行中);容器内 claude CLI 可用
- **边界定义**:
  - **做什么**:验收 R9.F1 既有行为——① 开终端自动进 claude TUI 且可见历史对话上下文;② AI 对话面板**生成中**开终端 → 拦截提示(避免双写同一会话);③ 退出 claude 落回 bash(R9.F1 既有)
  - **不做什么**:终端内渲染对话 UI;新增会话管理功能;改动 R9.F1 会话机制
- **字段定义**:无(零新表零接口;复用 tasks.claude_session_id)
- **交互规则**(R9.F1 既有,本点验收对象):
  | 场景/条件 | 行为/规则 | 结果/去向 |
  |---|---|---|
  | 打开终端(容器运行) | exec 包装自动进 claude,--session-id(首)/--resume(续) | TUI 带历史 |
  | AI 对话面板生成中 | 拦截开终端 | 提示文案 |
  | 退出 claude | 落回 bash | /bin/bash |
- **依赖**:R9.F1(实现)、R8(容器)、R5(对话)
- **异常与边界场景**:容器内无 claude → 维持 R9.F1 直接落 bash(不报错);会话 id 不存在(CLI 侧) → CLI 自身提示,平台不兜底
- **验收标准**:
  1. 对话面板发过消息后打开终端 → claude TUI 出现且**可见历史上下文**
...
- Runner 本地快速创建的跨机管理(仅本机)/ deploy 角色快速创建 / 机器重启后自动重拉(R31;~~Docker 容器形态启动~~ **已由 2026-09-24 用户指令落地为容器形态,见 R31.F3/BUG-049,移出范围外**)
- Runner 自由标签(GPU/大内存等资源标签)与「任务类型→tag」映射配置(R32 只做任务类型固定枚举)
- Runner role 可编辑 / 编辑弹窗改 token(R32)
- AI 对话斜杠:MCP **工具级**枚举(只做 server 级,R33)/ 平台技能解释器与编排(执行全归容器内 CLI,R33)/ 终端内斜杠补全 / 自定义斜杠命令管理界面(R33)
- 终端内渲染 AI 对话 UI / 新增会话管理(R34 验收确认型,零新代码)
...
- [x] R28 头像上传存储路径:`./data/avatars/{user_id}/` 是否合适?(向技术负责人确认)——**2026-09-24 自主确认:维持现状**(R28 已按此实施,DEVPLAN 增量4 Q41 留痕)
- [x] R29 导览步骤动态生成:复用现有 `/api/projects`、`/api/requirements`、`/api/tasks` 接口,还是需要聚合接口?——**2026-09-24 自主确认:复用现有接口前端链式取数**(R29 已实施,useTourSteps)
...
- **Q63–Q70**(2026-09-24 增量7,R33/R34):
  - Q63:`/` 菜单数据源 → **项目 Skills + 平台 Skills + 已启用 MCP server**,按类型分组
  - Q64:执行语义 → **原生透传**(斜杠原文随消息发容器内 claude CLI,平台零解释器)
  - Q65:MCP 粒度 → **V1 server 级**,工具级枚举 V2
  - Q66:交互形态 → **与 @ 文件补全同款**(按首字符 @ // 分流;空态不弹菜单)
  - Q67:范围 → **仅 AI 对话输入框**,终端不做补全
  - Q68:R34 定位 → **验收确认型**(Q6=A 用户未实测过;R9.F1 已实现,真实容器实测,缺陷转 rd-fix)
  - Q69:R34 形态 → 自动 `--resume` 进 TUI 带历史;**对话生成中开终端拦截**;退出落 bash
  - Q70:验收口径 → R33 四条(菜单/执行/空态),R34 三条(历史可见/双向续聊/生成中拦截);零新表,R33 数据全来自 R17
- **Q58–Q62**(2026-09-24 增量6,R32):
  - Q58:tag 模型 → **固定枚举 requirement/dev/test**,一 runner 多 tag;**空 tags=兜底接所有 worker 任务**(存量零变化);release 仍 role=deploy 不看 tag
  - Q59:编辑能力 → **除 role/token 外全字段可改**(名称唯一 16005/最大容器数调小允许存量不动/tags 多选;deploy 行加公网 IP)
  - Q60:R31 快速创建弹窗 → **同步加 tag 多选**(默认全不勾=兜底)
  - Q61:无匹配 runner → **8003 排队文案不变**,调度日志细分"等待 {类型} Runner"
  - Q62:tag 变更 → **新增 runner.update 审计**(detail 记前后 tags,无敏感值)
- **Q51–Q57**(2026-09-24 增量5,R31):
  - Q51:快速创建机制 → **平台后端 spawn 本机 runner 进程**(零表单 token 复制;与远程 token 流程并存;不依赖 runner 镜像构建)
  - Q52:deploy 角色 → **不纳入**(本入口恒 worker;deploy 走原 token 流程)
  - Q53:列表识别 → **is_local 标记 + 启动方式列/操作按钮差异**(停止/启动/重启仅本机型),不另加徽章
  - Q54:V1 部署承诺 → **仅平台源码方式运行**(能定位 runner/ 目录);环境不满足点击时细分报错,不支持平台容器化部署下的本功能
  - Q55:停止实现 → **指令式**(WS 下发 runner_shutdown,runner 优雅自退;平台重启后仍可停;停止超时 5s 且有进程句柄才强杀)
  - Q56:删除代停 → **仅本机 runner**:先停全部容器(R4 强制 push 后销毁)→ 停进程 → 删记录;push 失败整删中止。远程 runner 维持 16001
  - Q57:数量与表单 → **上限 3 个**(16004);最小弹窗=名称(默认 local-<短id>)+ 最大容器数(默认 10);**token 不显示**(自动注入;排障走重置 token)
...
- **Q28**(2026-09-21 增量2;2026-09-24 更名):品牌定名 **「旗橙」**(仅中文名,不设英文标识;原名「旗程」,用户定案更名且 Logo 换为橙子图标);预览/部署**根域名入平台设置**(`preview_base_domain` / `deploy_base_domain`,超管配置,示例值仅为默认)——域名与品牌解耦,平台上线时设置
